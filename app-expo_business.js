...
function markAccordionAsOpened(type) {
  if (type === 'imagem') accordionFlags.imagemAjustada = true;
...
let tarjaImg = null, tarjaCfg = null, categoriaSelecionada = 'empresa';
...
async function initCanvas() {
  ...
  try {
    ...
    console.error('Não foi possível carregar a tarja fixa:', e);
    tarjaImg = null;
  }
...
function gerarPost() {
  ...
  if (logoImg) {
    ...
    const logoCenterX = 657;
    const logoCenterY = 415;
  ...
  const tituloShiftDuasLinhas = -24;
  ...
  const centroY = canvas.height - 301;
